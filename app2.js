...
const cciExcelFilePath = path.join('C:\\Users\\admin\\paridhi\\excel sheet', 'CCI Calculation.xlsx');
...
// Helper to update LCR in CCI sheet based on output.json data
const updateLCRInCCI = async (outputData) => {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(cciExcelFilePath);
  const sheet = workbook.worksheets[0];

  for (const resource of outputData) {
    const { "GSO role": gsoRole, LCR } = resource;

    // Find the row in CCI sheet matching the GSO role
    let rowToUpdate;
    sheet.eachRow((row, rowNumber) => {
      if (row.getCell(3).value === gsoRole) {  // Assuming GSO role is in column 3 (adjust if necessary)
        rowToUpdate = row;
      }
    });

    if (rowToUpdate) {
      // Update LCR in the corresponding row (assuming LCR is in column 6)
      rowToUpdate.getCell(6).value = LCR; // Column 6: LCR

      // Save changes to Excel file
      await workbook.xlsx.writeFile(cciExcelFilePath);
    }
  }
...
// Helper to read the calculated CCI and CI values from CCI.xlsx
...
// Endpoint to filter data, update LCR, and let Excel calculate CCI and CI
app.post("/filter-and-calculate", async (req, res) => {
  try {
    ...
    // Update LCR values in the CCI sheet
    const updatedData = await updateLCRInCCI(finalData);

    // Get the calculated CCI and CI from CCI.xlsx
    const finalOutputData = await getCalculatedCCIAndCI(updatedData);

    // Save the updated data back to output.json
    fs.writeFileSync(outputFilePath, JSON.stringify(finalOutputData, null, 2));

    // Send the final data as a response
    res.json(finalOutputData);
  } catch (error) {
